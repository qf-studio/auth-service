...
func TestPostgresRARRepository_ResourceTypeCRUD(t *testing.T) {
	...
	// FindByID
	found, err := repo.FindResourceTypeByID(ctx, rt.ID)
	require.NoError(t, err)
	assert.Equal(t, rt.Type, found.Type)

	// FindByType
	found, err = repo.FindResourceTypeByType(ctx, "payment_initiation")
	require.NoError(t, err)
	assert.Equal(t, rt.ID, found.ID)

	// FindByType not found
	_, err = repo.FindResourceTypeByType(ctx, "nonexistent")
	...
	types, err := repo.ListResourceTypes(ctx)
	require.NoError(t, err)
	...
	// Update not found
	missing := &domain.RARResourceType{ID: uuid.New()}
	...
	// Delete
	err = repo.DeleteResourceType(ctx, rt.ID)
	require.NoError(t, err)

	_, err = repo.FindResourceTypeByID(ctx, rt.ID)
	assert.ErrorIs(t, err, storage.ErrNotFound)

	// Delete not found
	err = repo.DeleteResourceType(ctx, uuid.New())
	assert.ErrorIs(t, err, storage.ErrNotFound)
}
...
func TestPostgresRARRepository_ClientTypeAssociations(t *testing.T) {
	...
	types, err := repo.ListClientAllowedTypes(ctx, client.ID)
	require.NoError(t, err)
	assert.Empty(t, types)

	allowed, err := repo.IsClientTypeAllowed(ctx, client.ID, "payment_initiation")
	require.NoError(t, err)
	assert.False(t, allowed)

	// Allow types
	err = repo.AllowClientType(ctx, client.ID, rt1.ID)
	require.NoError(t, err)

	err = repo.AllowClientType(ctx, client.ID, rt2.ID)
	require.NoError(t, err)
	...
	allowed, err = repo.IsClientTypeAllowed(ctx, client.ID, "nonexistent")
	require.NoError(t, err)
	assert.False(t, allowed)

	// Revoke type
	err = repo.RevokeClientType(ctx, client.ID, rt1.ID)
	require.NoError(t, err)
	...
	err = repo.DeleteResourceType(ctx, rt2.ID)
	require.NoError(t, err)
