...
func startServer(t *testing.T, logger *zap.Logger, handler http.Handler, closers []httpserver.Closer) (*httpserver.Server, string) {
	t.Helper()
	srv := &http.Server{
		Addr:    ":0", // random port
		Handler: handler,
	}
	s := httpserver.New(logger, []*http.Server{srv}, closers)
...
func TestServer_ClosersCalledAfterHTTPDrain(t *testing.T) {
	...
	s := httpserver.New(logger,
		[]*http.Server{{Addr: ":0", Handler: slowHandler()}},
...
func TestServer_MultipleServers(t *testing.T) {
	...
	publicSrv := &http.Server{Addr: ":0", Handler: healthHandler()}
	adminSrv := &http.Server{Addr: ":0", Handler: healthHandler()}
